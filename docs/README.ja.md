...
- **壊れたリンクを「問題」パネルに出し、候補から選んで直せます。** 存在しない相対リンク・画像・`#見出し` アンカーを、開いている文書は自動で、ワークスペース内のすべての Markdown はコマンド 1 つで検査します
...
- 言語は文書から読みます — front matter の `lang:`（`language:` / `言語:` も可）があればそれ、無ければ本文の文字種から判定します。**どちらも入力だけで決まるので、GlyphCove が文書に差し込む文言は、変換を実行した人によって変わりません。**言語の手がかりが 1 つも無い文書（数値だけの表など）のときだけ、表示言語に倒します
- **変換について報告する文言は、実行者の表示言語に従います** — 変換ログ（`.log.txt`）・品質レポート・警告の文。これらは文書ではなく「その実行」を説明するものです
- ⚠ **Excel の場合、その一部はファイルの中に入ります。** `Markdown → Excel` は変換について報告するシート（`_Warnings` と、`_Assets` の理由欄）を足し、その文言は**実行者の**表示言語に従います。したがって、報告することがある変換では、同じファイルを別の人が変換すると**そのシートの中身が異なるブック**ができます。**`_Metadata` を含むそれ以外は文書側で決まり**、報告することが 1 つも無い変換なら、誰が変換しても同じブックになります。付けたくないときは設定 `glyphcove.xlsx.metaSheets` で切れます
...
**下の手引きは拡張機能に同梱しています**（日本語・英語の両方）。**本文**を読むのにネットワークは要りません — コマンドパレットで **「GlyphCove: 使用手引きを開く」** を実行して選んでください。⚠ 手引きの中の画面写真と、手引きの外へ出るリンクは Web を指したままです。下のリンクは、同じものの Web 版です。
...
- **無料のまま**: Markdown の編集・プレビュー・貼り付けと、Markdown からの HTML / PDF への基本出力
- **有償の Pro ライセンス（買い切り）の対象にする予定**: Word / Excel への出力（MD → DOCX / XLSX）、逆変換（PDF / Word / Excel / HTML → Markdown）とそれを土台にした 2 段変換、Word / Excel → HTML の直接変換、一括変換と結合、品質レポート、「Office 用にコピー」、印刷時の柱（ヘッダー/フッター・ページ番号）、透かし
...
- **サポートは公開の Issue で、日本語または英語で受け付けます**（第 16 条）。お問い合わせに使われた言語と同じ言語で回答します。電話・ビデオ会議・画面共有による対応と、お客様の環境の調査・設定の代行は行いません。**応答時間の約束もしていません**
